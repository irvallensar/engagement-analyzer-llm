...
def evaluate(filepath, max_samples=None):
    ...
    for i, data in enumerate(dataset):
        ...
        for idx, label in tok_tp: token_cat_tp[label] += 1
        for idx, label in tok_fp: token_cat_fp[label] += 1
        for idx, label in tok_fn: token_cat_fn[label] += 1
  ...
  # ----- CALCULATION OF PERFORMANCE METRIC SCORES ------
    ...
    for label in sorted(all_labels):
        ...
        # NEW: Accumulate running totals for Macro/Weighted
    ...
    for label in token_labels:
        ...
        macro_tok_f1_sum += l_f1
        weighted_tok_f1_sum += (l_f1 * support)

    # Calculate final Token averages
    t_macro_f1 = macro_tok_f1_sum / len(token_labels) if len(token_labels) > 0 else 0
    t_weighted_f1 = weighted_tok_f1_sum / total_tok_support if total_tok_support > 0 else 0
