            ...
            for p_span, score in zip(predicted_spans, span_scores):
                ...
                if score >= CONFIDENCE_THRESHOLD:
                    ...
                    if not span_tokens.intersection(gold_boundaries):
                        ...
                        accepted_by_label[p_span.label_] += 1
                    ...
                    rejected_by_label[p_span.label_] += 1
        else:
    ...
    if all_scores:
        try:
            ...
            for threshold in [0.5, 0.6, 0.7, 0.8, 0.9, 0.95]:
                kept = sum(s >= threshold for s in all_scores)
                print(
                    f"Threshold {threshold}: "
                    f"{kept}/{len(all_scores)} "
                    f"({100*kept/len(all_scores):.2f}%)"
                )
        except Exception as e:
            print(f"[WARNING] Could not compute score statistics: {e}")
            
    print("\n===== ACCEPTED LABELS =====")
    for label, count in accepted_by_label.most_common():
        print(f"{label}: {count}")
        
    print("\n===== REJECTED LABELS =====")
    for label, count in rejected_by_label.most_common():
        print(f"{label}: {count}")
