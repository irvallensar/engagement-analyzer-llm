...
# --- IOB PARSER (This is what was missing!) ---
def parse_iob_file(filepath):
    """Reads the IOB file and extracts sentences and Gold Spans."""
    with open(filepath, 'r', encoding='utf-8') as f:
        lines = f.readlines()
...
def evaluate(filepath, max_samples=None):
    ...
    # Slicing the dataset for quick tests
...
if __name__ == "__main__":
    # Currently set to run just the first 5 sentences for quick testing.
    evaluate("data/dev.iob", max_samples=5)            print(f"  Sentence: {data['text']}")
            print(f"  Gold Spans: {gold_spans}")
            print(f"  Pred Spans: {pred_spans}")
            print(f"  -> Errors: {fp} False Positives, {fn} False Negatives\n")
