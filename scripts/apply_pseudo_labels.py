...
# Strict threshold for capturing unprompted background/secondary labels.
...
def merge_and_qa_pseudo_labels(baseline_model_path, synthetic_input_path, output_path):
    ...
    for doc in raw_docs:
        ...
        for llm_span in llm_primary_spans:
            ...
            # GUARANTEED REF-PATTERN SAFETY NET:
            # If Qwen explicitly generated a CITATION and the text physically contains a 
            # bracketed number [X] or a 4-digit year, bypass teacher mismatch to preserve volume.
            if llm_span.label_ == "CITATION":
                if re.search(r'\[\d+\]|\b(19|20)\d{2}\b', doc.text):
                    match_found = True
                    verified_primary_spans.append(llm_span)
                    occupancy_map.update(range(llm_span.start, llm_span.end))
                    continue
            
            # TEACHER OVERLAP CHECK (With Cross-Category Family Flexibility)
            for i, p_span in enumerate(predicted_spans):
                valid_labels = [llm_span.label_]
                if llm_span.label_ == "CITATION":
                    valid_labels = ["CITATION", "SOURCES", "ATTRIBUTION"]
                elif llm_span.label_ == "SOURCES":
                    valid_labels = ["SOURCES", "CITATION", "ATTRIBUTION"]
                    
                if p_span.label_ in valid_labels:
                    # Character overlap check
                    if max(llm_span.start_char, p_span.start_char) < min(llm_span.end_char, p_span.end_char):
                        match_found = True
                        
                        # Reconstruct the span using the original targeted class balance label
                        verified_span = doc.char_span(
                            p_span.start_char, 
                            p_span.end_char, 
                            label=llm_span.label_, 
    ...
    print(f"\n[SUCCESS] Verified data saved to: {output_path}")
