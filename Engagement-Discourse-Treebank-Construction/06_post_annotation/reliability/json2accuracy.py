...
ENGAGEMENT = [
    'ATTRIBUTE', 'CONCUR', 'COUNTER', 'DENY', 'ENDORSE', 'ENTERTAIN',
    'PRONOUNCE', 'MONOGLOSS'
]
...
def extract_tags(reduced_list: list[list]):
    holder1 = []
    holder2 = []

    for sent in reduced_list:
        for token in sent:
            tag1 = token['tag1']
            tag2 = token['tag2']
            holder1.append(tag1)
...
def json2dict(json_list: list,
    ...
    eng_acc = simple_acc(eng_dict)
    print(eng_acc)

    ## Engagement confusion matrix
    eng_tag1, eng_tag2 = extract_tags(engmts)
    print(confusion_matrix(eng_tag1, eng_tag2))
    print(classification_report(eng_tag1, eng_tag2, labels=ENGAGEMENT))
    print(
        f"Cohen's kappa score: {cohen_kappa_score(eng_tag1, eng_tag2, labels=ENGAGEMENT)}"
    )

    print(
        f"Matthew's Correlation Coefficient: {matthews_corrcoef(eng_tag1, eng_tag2)}"
    )

    y_true = pd.Series(eng_tag1, name="Annotator1")
    y_pred = pd.Series(eng_tag2, name="Annotator2")
    df_confusion = pd.crosstab(y_true, y_pred)
    print(df_confusion)
    df_confusion.to_csv(
        f'data/benchmark_scores/eng_confusion_{anno_name1}-{anno_name2}_{batch_letter}_{regex_fileno}.csv'
    )

    ## Supplement confusion matrix
    spl_tag1, spl_tag2 = extract_tags(splmts)
    print(confusion_matrix(spl_tag1, spl_tag2))
    print(classification_report(spl_tag1, spl_tag2))

    y_true = pd.Series(spl_tag1, name="Annotator1")
    y_pred = pd.Series(spl_tag2, name="Annotator2")
    df_confusion = pd.crosstab(y_true, y_pred)
    print(df_confusion)
    df_confusion.to_csv(
        f'data/benchmark_scores/spl_confusion_{anno_name1}-{anno_name2}_{batch_letter}_{regex_fileno}.csv'
    )
